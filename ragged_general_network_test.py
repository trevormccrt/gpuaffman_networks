...
def test_ragged_k_state_update_extra_dims():
    ...
    true_functions = binary_core.random_binary_data((batch_size, N, 1<<k_true), 0.5)
    ...
    extra_functions = binary_core.random_binary_data((batch_size, N, 1<<k_cont - 1<<k_true), 0.5)
...
def test_ragged_k_state_update_mixed_dims():
    ...
    N = 20
    k_vals = [3, 5]
    states = binary_core.random_binary_data((batch_size, 2, N), 0.5)
    functions = binary_core.random_binary_data((batch_size, 2, N, 1 << np.max(k_vals)), 0.5)
...
def test_cuda():
    ...
    k_max = 8
    states = binary_core.random_binary_data((batch_size, 2, N), 0.5)
    functions = binary_core.random_binary_data((batch_size, 2, N, 1 << k_max), 0.5)
